...
export function AdminHeader({ session, onMenu }: { session: StaffSession; onMenu: () => void }) {
  return (
    <header className="flex flex-wrap items-center justify-between gap-4 border-b border-[#0d2922]/10 bg-white px-5 py-4 lg:px-8">
      <div className="flex items-center gap-3">
        <button type="button" onClick={onMenu} className="rounded-lg border border-[#0d2922]/20 px-3 py-2 lg:hidden" aria-label="Yönetim menüsünü aç">Menü</button>
        <div>
