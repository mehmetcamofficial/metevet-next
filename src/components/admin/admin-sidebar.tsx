...
export function AdminSidebar({ role, open, onClose }: { role: UserRole; open: boolean; onClose: () => void }) {
  ...
  const content = <aside className="flex h-full w-72 max-w-[88vw] flex-col overflow-y-auto bg-[#0d2922] px-5 py-7 text-white lg:w-64">
    ...
    <nav aria-label="Yönetim menüsü" className="mt-7 flex flex-col gap-1">{shared.map((link) => { const active = link.href === "/admin" ? pathname === link.href : pathname.startsWith(link.href); return <Link key={link.href} href={link.href} onClick={onClose} aria-current={active ? "page" : undefined} className={`rounded-lg px-3 py-2.5 text-sm transition focus-visible:outline-2 focus-visible:outline-[#cda85f] ${active ? "bg-white text-[#0d2922]" : "text-white/85 hover:bg-white/10"}`}>{link.label}</Link>; })}{role==="admin"?<>{[["/admin/staff","Personel"],["/admin/settings","Ayarlar"],["/admin/audit-log","Audit Log"]].map(([href,label])=>{const active=pathname.startsWith(href);return <Link key={href} href={href} onClick={onClose} aria-current={active?"page":undefined} className={`rounded-lg px-3 py-2.5 text-sm ${active?"bg-white text-[#0d2922]":"text-white/85 hover:bg-white/10"}`}>{label}</Link>})}</>:null}{(()=>{const active=pathname.startsWith("/admin/profile");return <Link href="/admin/profile" onClick={onClose} aria-current={active?"page":undefined} className={`rounded-lg px-3 py-2.5 text-sm ${active?"bg-white text-[#0d2922]":"text-white/85 hover:bg-white/10"}`}>Profilim</Link>})()}{comingSoon[role].map((label) => <span key={label} aria-disabled="true" className="flex cursor-not-allowed items-center justify-between rounded-lg px-3 py-2.5 text-sm text-white/45"><span>{label}</span><span className="text-[10px] uppercase">Yakında</span></span>)}</nav>
  </aside>;
