...
import { getVetAppointments, getVetMetrics, getNextPatient } from "@/src/lib/admin/veterinarian/veterinarian-readers";
...
export default async function VeterinarianPage({
  ...
  // Veterinarian defaults to own ID; admin can inspect others
  const veterinarianId = session.profile.role === "admin" && p.veterinarian_id
    ? p.veterinarian_id
    : session.id;
  ...
  return (
    <AdminShell session={session}>
      ...
      <VeterinarianToolbar date={date} previous={shiftDate(date, -1)} next={shiftDate(date, 1)} today={today} />

      <div className="mt-4">
        <VeterinarianWorkspace appointments={appointments} metrics={metrics} nextPatient={nextPatient} />
      </div>
    </AdminShell>
  );
}
