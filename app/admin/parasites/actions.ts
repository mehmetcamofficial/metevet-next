...
async function save(id:string|null,_state:ParasiteState,fd:FormData):Promise<ParasiteState>{
  ...
  const s=await createServerActionClient();if(!s)return{message:"İşlem başarısız."};
...
async function life(id:string,action:"archive"|"restore"|"delete"){const session=await requireStaff();if(!canPermanentlyDelete(session.profile.role))throw new Error("Yetkiniz yok.");const s=await createServerActionClient();if(!s)throw new Error("İşlem başarısız.");const{data}=await s.from("parasite_records").select("pet_id").eq("id",id).single();if(!data)throw new Error("Kayıt yok.");const result=action==="delete"?await s.from("parasite_records").delete().eq("id",id):await s.from("parasite_records").update({archived_at:action==="archive"?new Date().toISOString():null}).eq("id",id);if(result.error)throw new Error("İşlem başarısız.");await s.from("audit_logs").insert({actor_user_id:session.id,action:`parasite_${action}d`,entity_type:"parasite_record",entity_id:id,metadata:{}});revalidatePath("/admin/parasites");revalidatePath(`/admin/pets/${data.pet_id}`);if(action==="delete")redirect("/admin/parasites")}
